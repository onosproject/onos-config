...
// A plugin for the YGOT model of Junos-19.3R1.
package main

import (
	"fmt"
	"github.com/onosproject/onos-config/modelplugin/Junos-19.3R1/junos_19_3R1"
...
const modelversion = "19.3R1"
const modulename = "junos.so.19.3R1"
...
func (m modelplugin) UnmarshalConfigValues(jsonTree []byte) (*ygot.ValidatedGoStruct, error) {
	device := &junos_19_3R1.Device{}
	vgs := ygot.ValidatedGoStruct(device)

	if err := junos_19_3R1.Unmarshal([]byte(jsonTree), device); err != nil {
		return nil, err
	}
...
func (m modelplugin) Validate(ygotModel *ygot.ValidatedGoStruct, opts ...ygot.ValidationOption) error {
	...
	device, ok := deviceDeref.(*junos_19_3R1.Device)
	if !ok {
		return fmt.Errorf("unable to convert model in to junos_19_3R1")
	}
	return device.Validate()
}

func (m modelplugin) Schema() (map[string]*yang.Entry, error) {
	return junos_19_3R1.UnzipSchema()
}
