...
import (
	...
	snapshotstore "github.com/onosproject/onos-config/pkg/store/network/snapshot"
	"github.com/onosproject/onos-config/pkg/types"
	snapshottype "github.com/onosproject/onos-config/pkg/types/network/snapshot"
	"sync"
)
...
type Watcher struct {
	...
	ch    chan *snapshottype.NetworkSnapshot
	mu    sync.Mutex
}
...
func (w *Watcher) Start(ch chan<- types.ID) error {
	...
	requestCh := make(chan *snapshottype.NetworkSnapshot, queueSize)
	w.ch = requestCh
	w.mu.Unlock()
