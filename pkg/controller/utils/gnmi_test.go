...
import (
	...
	configv3 "github.com/onosproject/onos-api/go/onos/config/v3"
	"reflect"
	"testing"
...
func Test_GnmiStringToNative(t *testing.T) {
	...
	nativeType, err := NewTypedValue(&gnmi.TypedValue{Value: &gnmiValue}, nil)
	assert.NoError(t, err)

	nativeString := (*configv3.TypedString)(nativeType)
	assert.Equal(t, nativeString.String(), testString)
}

func Test_GnmiIntToNative(t *testing.T) {
	pathElem := adminapi.ReadWritePath{
		ValueType: configv2.ValueType_INT,
		TypeOpts:  []uint64{uint64(configv3.WidthThirtyTwo)},
	}
	gnmiValue := gnmi.TypedValue_IntVal{IntVal: testNegativeInt}
	nativeType, err := NewTypedValue(&gnmi.TypedValue{Value: &gnmiValue}, &pathElem)
	assert.NoError(t, err)

	nativeInt64 := (*configv3.TypedInt)(nativeType)
	assert.Equal(t, nativeInt64.Int(), testNegativeInt)
}

func Test_GnmiUintToNative(t *testing.T) {
	pathElem := adminapi.ReadWritePath{
		ValueType: configv2.ValueType_UINT,
		TypeOpts:  []uint64{uint64(configv3.WidthSixtyFour)},
	}
	gnmiValue := gnmi.TypedValue_UintVal{UintVal: uint64(testMaxUint)}
	nativeType, err := NewTypedValue(&gnmi.TypedValue{Value: &gnmiValue}, &pathElem)
	assert.NoError(t, err)

	nativeUint64 := (*configv3.TypedUint)(nativeType)
...
func Test_GnmiBoolToNative(t *testing.T) {
	...
	nativeBool := (*configv3.TypedBool)(nativeType)
...
func Test_comparables(t *testing.T) {
	testCases := []struct {
		description  string
		expectedType configv3.ValueType
		testValue    *gnmi.TypedValue
	}{
		{description: "Int List", expectedType: configv3.ValueType_LEAFLIST_INT, testValue: intListTestValue},
		{description: "Uint List", expectedType: configv3.ValueType_LEAFLIST_UINT, testValue: uintListTestValue},
		{description: "Decimal List", expectedType: configv3.ValueType_LEAFLIST_DECIMAL, testValue: decimalListTestValue},
		{description: "Boolean List", expectedType: configv3.ValueType_LEAFLIST_BOOL, testValue: booleanListTestValue},
		{description: "Float List", expectedType: configv3.ValueType_LEAFLIST_FLOAT, testValue: floatListTestValue},
		{description: "Bytes List", expectedType: configv3.ValueType_LEAFLIST_BYTES, testValue: bytesListTestValue},
		{description: "Strings List", expectedType: configv3.ValueType_LEAFLIST_STRING, testValue: stringListTestValue},
		{description: "Bytes Leaf", expectedType: configv3.ValueType_BYTES, testValue: bytesLeafTestValue},
		{description: "Float Leaf", expectedType: configv3.ValueType_FLOAT, testValue: floatLeafTestValue},
		{description: "Decimal Leaf", expectedType: configv3.ValueType_DECIMAL, testValue: decimalLeafTestValue},
	}

	for _, testCase := range testCases {
		nativeType, err := NewTypedValue(testCase.testValue, nil)
		assert.NoError(t, err)
...
func Test_ascii(t *testing.T) {
	nativeType, err := NewTypedValue(asciiLeafTestValue, nil)
	assert.NoError(t, err)
	assert.NotNil(t, nativeType)
	assert.Equal(t, nativeType.Type, configv3.ValueType_STRING)
...
func Test_asciiList(t *testing.T) {
	nativeType, err := NewTypedValue(asciiListTestValue, nil)
	assert.NoError(t, err)
	assert.NotNil(t, nativeType)
	assert.Equal(t, nativeType.Type, configv3.ValueType_LEAFLIST_STRING)
...
func Test_empty(t *testing.T) {
	convertedValue, convertedErr := NativeTypeToGnmiTypedValue(configv3.NewTypedValueEmpty())
	assert.NoError(t, convertedErr)
	s := convertedValue.String()
	fmt.Println(s)
...
func Test_errors(t *testing.T) {
	...
	badTypedValue := configv3.NewTypedValueEmpty()
...
func Test_NativeStringToGnmi(t *testing.T) {
	nativeString := configv3.NewTypedValueString(testString)
...
func Test_NativeIntToGnmi(t *testing.T) {
	nativeInt := configv3.NewTypedValueInt(testPositiveInt, 64)
...
func Test_NativeUintToGnmi(t *testing.T) {
	nativeUint := configv3.NewTypedValueUint(testMaxUint, 64)
...
func Test_NativeBoolToGnmi(t *testing.T) {
	nativeBool := configv3.NewTypedValueBool(true)
