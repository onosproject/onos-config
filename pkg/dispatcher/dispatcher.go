...
type Dispatcher struct {
	deviceListeners        map[devicetype.ID]chan events.ConfigEvent
	nbiListeners           map[string]chan events.ConfigEvent
	nbiOpStateListeners    map[string]chan events.OperationalStateEvent
	deviceResponseListener map[devicetype.ID]chan events.DeviceResponse
}

// NewDispatcher creates and initializes a new event dispatcher
func NewDispatcher() *Dispatcher {
	return &Dispatcher{
		deviceListeners:        make(map[devicetype.ID]chan events.ConfigEvent),
		...
		deviceResponseListener: make(map[devicetype.ID]chan events.DeviceResponse),
	}
}
...
func (d *Dispatcher) Listen(changeChannel <-chan events.ConfigEvent) {
	...
	for configEvent := range changeChannel {
		...
		deviceChan, ok := d.deviceListeners[devicetype.ID(configEvent.Subject())]
		if !ok {
...
func (d *Dispatcher) RegisterDevice(id devicetype.ID) (chan events.ConfigEvent, chan events.DeviceResponse, error) {
...
func (d *Dispatcher) UnregisterDevice(id devicetype.ID) error {
...
func (d *Dispatcher) HasListener(name devicetype.ID) bool {
	_, ok := d.deviceListeners[name]
	return ok
}

// GetResponseListener returns the response listener if the named listeners has been registered
func (d *Dispatcher) GetResponseListener(name devicetype.ID) (chan events.DeviceResponse, bool) {
