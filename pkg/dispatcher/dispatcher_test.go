...
var (
	...
	device1, device2, device3                      devicetype.Device
	err                                            error
)
...
func TestMain(m *testing.M) {
	...
	device1 = devicetype.Device{ID: "localhost-1", Address: "localhost:10161"}
	device2 = devicetype.Device{ID: "localhost-2", Address: "localhost:10162"}
	device3 = devicetype.Device{ID: "localhost-3", Address: "localhost:10163"}
...
func Test_register(t *testing.T) {
	d := setUp()
	device4Channel, respChannel4, err := d.RegisterDevice(devicetype.ID("device4"))
	...
	_ = d.UnregisterDevice(devicetype.ID("device4"))
	...
	_ = d.UnregisterDevice(devicetype.ID("opStateTest2"))
...
func Test_unregister(t *testing.T) {
	d := setUp()
	err1 := d.UnregisterDevice(devicetype.ID("device5"))
	...
	_, _, _ = d.RegisterDevice(devicetype.ID("device6"))
	_, _, _ = d.RegisterDevice(devicetype.ID("device7"))

	err2 := d.UnregisterDevice(devicetype.ID("device6"))
	assert.NilError(t, err2, "Unexpected error when unregistering device6 %s", err2)
	_ = d.UnregisterDevice(devicetype.ID("device7"))
...
func Test_listen_device(t *testing.T) {
	...
	for i := 1; i < 3; i++ {
		event := events.NewConfigEvent(device1.Address, changeID, true)
...
func Test_listen_nbi(t *testing.T) {
	...
	// Send down some changes
	for i := 1; i < 3; i++ {
		event := events.NewConfigEvent("foobar", changeID, true)
		changesChannel <- event
	}
...
func Test_register_dup(t *testing.T) {
	...
	_, _, _ = d.RegisterDevice(devicetype.ID("dev1"))
