...
func (m *Manager) GetTargetNewConfig(target string, version string, path string, layer int) ([]*types.PathValue, error) {
	log.Infof("Getting config for %s at %s", target, path)
	configValues, errExtract := device.ExtractFullConfig(devicetopo.ID(target), version, nil, m.DeviceChangesStore, layer)
	if errExtract != nil {
