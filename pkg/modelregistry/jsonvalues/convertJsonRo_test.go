...
func Test_correctJsonPathValues(t *testing.T) {
	...
	val01 := devicechange.PathValue{Path: systemNtpAuthMismatchNoNs,
		Value: devicechange.NewTypedValueFloat(systemNtpAuthMismatchValue)}

	const systemNtpEnableAuth = "/system/ntp/state/enable-ntp-auth"
	const systemNtpEnableAuthValue = true
	val02 := devicechange.PathValue{Path: systemNtpEnableAuth,
		Value: devicechange.NewTypedValueBool(systemNtpEnableAuthValue)}

	const systemNtpSourceAddr = "/system/ntp/state/ntp-source-address"
	const systemNtpSourceAddrValue = "192.168.0.17"
	val03 := devicechange.PathValue{Path: systemNtpSourceAddr,
		Value: devicechange.NewTypedValueString(systemNtpSourceAddrValue)}

	const iface1Mtu = "/interfaces/interface[1]/state/mtu"
	const iface1MtuValue = 960.0
	val04 := devicechange.PathValue{Path: iface1Mtu,
		Value: devicechange.NewTypedValueFloat(iface1MtuValue)}
	const iface1Name = "/interfaces/interface[1]/name"
	const iface1NameValue = "eth1"
	val05 := devicechange.PathValue{Path: iface1Name,
		Value: devicechange.NewTypedValueString(iface1NameValue)}
	const iface1Desc = "/interfaces/interface[1]/state/description"
	const iface1DescValue = "new if desc"
	val06 := devicechange.PathValue{Path: iface1Desc,
		Value: devicechange.NewTypedValueString(iface1DescValue)}
	...
	val07 := devicechange.PathValue{Path: iface1Sub15IfaceIdx,
		Value: devicechange.NewTypedValueFloat(iface1Sub15IfaceIdxValue)}
	const iface1Sub15Idx = "/interfaces/interface[1]/subinterfaces/subinterface[15]/index"
	const iface1Sub15IdxValue = "120"
	val08 := devicechange.PathValue{Path: iface1Sub15Idx,
		Value: devicechange.NewTypedValueString(iface1Sub15IdxValue)}
	...
	val09 := devicechange.PathValue{Path: iface1Sub15AdSt,
		Value: devicechange.NewTypedValueString(iface1Sub15AdStValue)}
	...
	val10 := devicechange.PathValue{Path: sysAaaGr10Svr199Ca,
		Value: devicechange.NewTypedValueFloat(sysAaaGr10Svr199CaValue)}
	const sysAaaGr10Name = "/system/aaa/server-groups/server-group[10]/name"
	const sysAaaGr10NameValue = "g1"
	val11 := devicechange.PathValue{Path: sysAaaGr10Name,
		Value: devicechange.NewTypedValueString(sysAaaGr10NameValue)}
	...
	val12 := devicechange.PathValue{Path: sysAaaGr10Svr199Addr,
		Value: devicechange.NewTypedValueString(sysAaaGr10Svr199AddrValue)}
	...
	val13 := devicechange.PathValue{Path: sysAaaGr12Svr199Ca,
		Value: devicechange.NewTypedValueFloat(sysAaaGr12Svr199CaValue)}
	const sysAaaGr12Name = "/system/aaa/server-groups/server-group[12]/name"
	const sysAaaGr12NameValue = "g2"
	val14 := devicechange.PathValue{Path: sysAaaGr12Name,
		Value: devicechange.NewTypedValueString(sysAaaGr12NameValue)}
	...
	val15 := devicechange.PathValue{Path: sysAaaGr12Svr199Addr,
		Value: devicechange.NewTypedValueString(sysAaaGr12Svr199AddrValue)}
	...
	val16 := devicechange.PathValue{Path: sysAaaGr12Svr200Ca,
		Value: devicechange.NewTypedValueFloat(sysAaaGr12Svr200CaValue)}
	...
	val17 := devicechange.PathValue{Path: sysAaaGr12Svr200Addr,
		Value: devicechange.NewTypedValueString(sysAaaGr12Svr200AddrValue)}
	jsonPathValues := []*devicechange.PathValue{&val01, &val02, &val03, &val04,
	...
	for _, correctedPathValue := range correctedPathValues {
		...
		case systemNtpAuthMismatchNoNs:
			assert.Equal(t, correctedPathValue.GetValue().GetType(), devicechange.ValueType_UINT)
			assert.Equal(t, len(correctedPathValue.GetValue().GetTypeOpts()), 0)
			assert.Equal(t, (*devicechange.TypedUint64)(correctedPathValue.GetValue()).Uint(), uint(systemNtpAuthMismatchValue))
		case systemNtpEnableAuth:
			assert.Equal(t, correctedPathValue.GetValue().GetType(), devicechange.ValueType_BOOL)
			assert.Equal(t, len(correctedPathValue.GetValue().GetTypeOpts()), 0)
			assert.Equal(t, (*devicechange.TypedBool)(correctedPathValue.GetValue()).Bool(), true)
		case systemNtpSourceAddr:
			assert.Equal(t, correctedPathValue.GetValue().GetType(), devicechange.ValueType_STRING)
			assert.Equal(t, len(correctedPathValue.GetValue().GetTypeOpts()), 0)
			assert.Equal(t, correctedPathValue.GetValue().ValueToString(), systemNtpSourceAddrValue)
		case ifEth1Mtu:
			assert.Equal(t, correctedPathValue.GetValue().GetType(), devicechange.ValueType_UINT)
			assert.Equal(t, len(correctedPathValue.GetValue().GetTypeOpts()), 0)
			assert.Equal(t, (*devicechange.TypedUint64)(correctedPathValue.GetValue()).Uint(), uint(iface1MtuValue))
		case ifEth1Desc:
			assert.Equal(t, correctedPathValue.GetValue().GetType(), devicechange.ValueType_STRING)
			assert.Equal(t, len(correctedPathValue.GetValue().GetTypeOpts()), 0)
		...
		case ifEth1Sub120IfIdx:
			assert.Equal(t, correctedPathValue.GetValue().GetType(), devicechange.ValueType_UINT)
			assert.Equal(t, len(correctedPathValue.GetValue().GetTypeOpts()), 0)
			assert.Equal(t, (*devicechange.TypedUint64)(correctedPathValue.GetValue()).Uint(), uint(iface1Sub15IfaceIdxValue))
		...
		case sysAaaGrG1Srv7Ca:
			assert.Equal(t, correctedPathValue.GetValue().GetType(), devicechange.ValueType_UINT)
			assert.Equal(t, len(correctedPathValue.GetValue().GetTypeOpts()), 0)
			assert.Equal(t, (*devicechange.TypedUint64)(correctedPathValue.GetValue()).Uint(), uint(sysAaaGr10Svr199CaValue))
		case sysAaaGrG2Srv4Ca:
			assert.Equal(t, correctedPathValue.GetValue().GetType(), devicechange.ValueType_UINT)
			assert.Equal(t, len(correctedPathValue.GetValue().GetTypeOpts()), 0)
			assert.Equal(t, (*devicechange.TypedUint64)(correctedPathValue.GetValue()).Uint(), uint(sysAaaGr12Svr199CaValue))
		case sysAaaGrG2Srv5Ca:
			assert.Equal(t, correctedPathValue.GetValue().GetType(), devicechange.ValueType_UINT)
			assert.Equal(t, len(correctedPathValue.GetValue().GetTypeOpts()), 0)
			assert.Equal(t, (*devicechange.TypedUint64)(correctedPathValue.GetValue()).Uint(), uint(sysAaaGr12Svr200CaValue))
		default:
