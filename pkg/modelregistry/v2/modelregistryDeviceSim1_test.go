...
func Test_SchemaDeviceSim(t *testing.T) {
	...
	assert.Equal(t, logSvrSelFacName.ValueType, configapi.ValueType_STRING, "expected /system/logging/remote-servers/remote-server[host=*]/selectors/selector[facility=*][severity=*]/config/facility to be STRING")
