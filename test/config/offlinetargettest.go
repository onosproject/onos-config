...
func (s *TestSuite) TestOfflineTarget(t *testing.T) {
	...
	targetPath := gnmi.GetTargetPathWithValue(offlineTargetName, modPath, modValue, proto.StringVal)
	gnmi.SetGNMIValueOrFail(t, gnmiClient, targetPath, gnmi.NoPaths, []*gnmi_ext.Extension{})
	...
	gnmi.WaitForTargetAvailable(t, topoapi.ID(simulator.Name()), time.Minute)
	gnmi.CheckGNMIValue(t, gnmiClient, targetPath, modValue, 0, "Query after set returned the wrong value")
	...
	gnmi.CheckTargetValue(t, targetGnmiClient, targetPath, modValue)
